...
def clockifyImportPayroll(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # CSV column names separate from model field names
            source_columns = list(file.columns)

            # list: (csv_column, model_field)
            mapped_columns = []
                ...
                with transaction.atomic():
                    for line in data:
                        if 'payperiod' not in line:
                            raise ValidationError({"payperiod": "No pay period for this date range"})
